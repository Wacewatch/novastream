...
_DADDY_HOST_ALLOW_RE = re.compile(
    ...
    r"|tvdaily\.|phantemlis\.|romponalis\.|newkso\.|dlstream\.)",
    re.I,
)
...
async def daddy_proxy(request: Request, url: str = Query("")):
    ...
    if not _DADDY_HOST_ALLOW_RE.search(parsed.netloc):
        # Allow if host ends with a known DLStream-rotation TLD to avoid
        # breaking whenever the CDN swaps domains (they rotate constantly:
        # .sbs/.shop/.online/.xyz/.top/.st/.space/.click/.site/.pro/.live/.fun/.cc).
        if not re.search(r"\.(sbs|shop|online|space|click|site|xyz|top|st|pro|live|fun|cc|icu|lat|beer)$", parsed.netloc, re.I):
    ...
    _seg_host = any(h in parsed.netloc for h in ("chat.cfbu247.sbs", "zampledakis", "tvdaily", "phantemlis", "romponalis"))
    _seg_path = "/segment" in parsed.path.lower()
    if _DADDY_SEG_EXT_RE.search(parsed.path) or _seg_host or _seg_path:
...
JACKTV_DEFAULTS: Dict[str, Any] = {
    "site_url": "https://jack09eo.mpcourageny9i9zzipper.my",
