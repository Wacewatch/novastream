...
# IMPORTANT: vavoo.tv MUST be tried first. As of 2026 the situation reversed:
# the lokke.app ping now returns a "free-tier" signature that makes the upstream
# serve a ~81KB placeholder/ad slate ("FFmpeg Service01") instead of the real
# channel, while the vavoo.tv ping returns the working signature (real ~3-4 MB
# segments). This matches the reference vavoo-iptv-stream-proxy which uses
# vavoo.tv only. lokke.app is kept only as a last-resort fallback.
...
async def download_project_zip():
    """Serve the packaged source code (no node_modules / .git / .env)."""
    from fastapi.responses import FileResponse as _FileResponse
    zip_path = Path("/app/backend/downloads/livewatch-project.zip")
    if not zip_path.exists():
        raise HTTPException(status_code=404, detail="Archive introuvable")
    return _FileResponse(
        str(zip_path),
        media_type="application/zip",
        filename="livewatch-project.zip",
    )
