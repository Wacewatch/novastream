...
# --------------------------------------------------------------------- #
JACK07_SITE_DEFAULT = "https://jack09eo.mpcourageny9i9zzipper.my"
...
def set_site_url(url: str) -> str:
    ...
    JACK07_SITE = url
    return JACK07_SITE
# Upstream API/CDN base domain. The front-facing site (jack09eo.*.my /
# fctv33hd.beer) rotates several times a day, but the API + static CDN live on
# a more stable "tc*.ru" base domain that the SPA bundle points at. When the
# provider swaps this base (a few times a year) update JUST this one constant.
# 2026-07: fctv33hd.beer / jack09eo.* now serve from tcfru133dd3c.ru
# (previous base was tcore131ybdf.ru).
JACK07_API_BASE = "tcfru133dd3c.ru"
# Upstream API hosts (try each in order — the first responsive one wins).
JACK07_API_HOSTS = [
    f"https://apis-data10.{JACK07_API_BASE}",
    f"https://apis-data-defra10.{JACK07_API_BASE}",
]
...
# a *previous* CDN hostname that no longer resolves. We rewrite known stale
# hosts to the current active CDN so logos render in the SPA.
# 2026-07 active CDN base: tcfru133dd3c.ru (logos + statics).
_LOGO_HOST_REWRITES = {
    "logos1.tcrbg61levl.cfd": "logos1." + JACK07_API_BASE,
    "logos2.tcrbg61levl.cfd": "logos1." + JACK07_API_BASE,
    "logos3.tcrbg61levl.cfd": "logos1." + JACK07_API_BASE,
    "logos1.tcore131ybdf.ru": "logos1." + JACK07_API_BASE,
    "logos2.tcore131ybdf.ru": "logos1." + JACK07_API_BASE,
    "logos3.tcore131ybdf.ru": "logos1." + JACK07_API_BASE,
}
