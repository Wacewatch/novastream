 ...
 * Standalone BossTV player — picks server[idx] (default 0) from
 * /api/bosstv/streams and plays it through the HLS VideoPlayer behind
 * the ad-unlock gate. No source/server name is exposed.
 */
export default function BossTvEmbedPage() {
    ...
    (async () => {
      try {
        const [m, s] = await Promise.all([
          axios.get(`${API}/bosstv/matches`),
          axios.get(`${API}/bosstv/streams`, { params: { mid: matchId } }),
        ]);
        if (cancelled) return;
  ...
  return (
    <div className="fixed inset-0 bg-black">
      ...
      {unlocked && streamUrl && (
        <VideoPlayer
          channel={{ id: match.id, name: match.title, country_code: "boss" }}
          streamUrl={streamUrl}
