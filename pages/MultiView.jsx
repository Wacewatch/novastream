...
const SOURCE_TABS = [
  { id: "tv", label: "LiveTV", icon: Tv2, color: "#ff2e63" },
  { id: "frame", label: "FrameTV", icon: Tv2, color: "#00d2ff" },
  { id: "north", label: "NorthTV", icon: Tv2, color: "#6c5ce7" },
...
export default function MultiView() {
  ...
  const buildSrc = (source, item) => {
    ...
    if (source === "north") return `/embed/north/${encodeURIComponent(item.slug || item.id)}`;
    if (source === "frame") return `/embed/frame/${encodeURIComponent(item.id)}`;
    return "";
  };
    ...
    (async () => {
      ...
      try {
        if (pickerSource === "tv") {
          ...
          setItems(list);
        } else if (pickerSource === "north") {
          const params = { country: country || "France", limit: 800 };
          if (search.trim()) params.search = search.trim();
          const r = await axios.get(`${API}/north/channels`, { params, signal: ctrl.signal });
          if (cancelled) return;
          const list = (r.data.channels || []).map((c) => ({
            kind: "north",
            id: c.id, slug: c.slug || c.id,
            name: c.name, country: c.country, category: c.category,
            src: buildSrc("north", c),
          }));
          setItems(list);
        } else if (pickerSource === "frame") {
          const params = { group: country || "France" };
          if (search.trim()) params.search = search.trim();
          const r = await axios.get(`${API}/frame/channels`, { params, signal: ctrl.signal });
          if (cancelled) return;
          const list = (r.data.channels || []).map((c) => ({
            kind: "frame", id: c.id, name: c.name, country: c.group, src: buildSrc("frame", c),
          }));
          setItems(list);
  ...
  return (
    <div className="relative min-h-screen text-white flex flex-col">
      ...
      {/* Grid */}
      <main className="flex-1 w-full mx-auto px-3 sm:px-5 lg:px-8 py-4">
        <div
          className="mv-grid"
          style={gridStyle}
