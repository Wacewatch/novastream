...
import {
  ...
  Flame, Crown, Zap, History, X, Clock,
} from "lucide-react";
...
import NorthTvTab from "@/components/tabs/NorthTvTab";
import FrameTvTab from "@/components/tabs/FrameTvTab";
...
export default function NovaStream() {
  // ---------- Top-level tab state ----------
  const [activeTab, setActiveTab] = useState(() => LS.get("livewatch.tab", "tv"));        // tv | daddy | sports
  const [sportsSubTab, setSportsSubTab] = useState("sports"); // sports | football | info

  // ---------- TV (Vavoo) state ----------
  const [countries, setCountries] = useState(["France"]);
  const [country, setCountry] = useState(() => LS.get("livewatch.country", "France"));
  const [category, setCategory] = useState(() => LS.get("livewatch.category", "Tous"));
  ...
  const { recent, record: recordWatch, remove: removeRecent, clear: clearRecent } = useRecentlyWatched();

  // ---------- North / Frame players ----------
  const [northActive, setNorthActive] = useState(null); // { slug, name, iframe_url, stream_url }
  const [frameActive, setFrameActive] = useState(null); // { id, name, stream_url }
  ...
  // ---------- Persistence: remember last tab / country / category ----------
  useEffect(() => { LS.set("livewatch.tab", activeTab); }, [activeTab]);
  useEffect(() => { LS.set("livewatch.country", country); }, [country]);
  useEffect(() => { LS.set("livewatch.category", category); }, [category]);
  ...
  const handlePickTv = (ch) => { recordWatch(ch); setPending({ kind: "tv", payload: ch }); };
  ...
  const handlePickNorth = (ch) => {
    setPending({ kind: "north", payload: ch });
  };

  const handlePickFrame = (ch) => {
    setPending({ kind: "frame", payload: ch });
  };
  ...
  const handleUnlocked = async () => {
    ...
    if (kind === "north") {
      ...
      setResolving(true);
      try {
        const r = await axios.get(`${API}/north/stream/${encodeURIComponent(ch.slug || ch.id)}`);
        setNorthActive({
          slug: ch.slug || ch.id,
          name: ch.name,
          stream_url: r.data?.stream_url || null,
          iframe_url: r.data?.iframe_url || null,
        });
      } catch (e) {
        console.error(e);
        toast.error("Flux NorthTV indisponible");
      } finally {
        setResolving(false);
      }
      return;
    ...
    if (kind === "frame") {
      ...
      setResolving(true);
      try {
        const r = await axios.get(`${API}/frame/stream/${encodeURIComponent(ch.id)}`);
        if (r.data?.stream_url) {
          setFrameActive({ id: ch.id, name: ch.name, stream_url: r.data.stream_url });
        } else {
          toast.error("Flux FrameTV indisponible");
        }
      } catch (e) {
        console.error(e);
        toast.error("Flux FrameTV indisponible");
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      {/* ===== Hub buttons (5 tabs) ===== */}
      <section className="w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 pt-6 pb-2" data-testid="hub-section">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="text-white font-extrabold text-lg leading-tight">LiveTV</div>
          ...
          <button
            ...
            data-testid="hub-frame"
            className={`hub-card text-left ${activeTab === "frame" ? "is-active" : ""}`}
          >
            <div className="hub-icon" style={{ background: "linear-gradient(135deg,#00d2ff 0%,#0ea5e9 100%)" }}>
              <Tv2 size={24} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-white font-extrabold text-lg leading-tight">FrameTV</div>
              <div className="text-white/55 text-xs mt-0.5 truncate">Chaînes en direct</div>
            </div>
            <span className="hub-chev">›</span>
          </button>
          ...
          <button
            ...
            data-testid="hub-north"
            className={`hub-card text-left ${activeTab === "north" ? "is-active" : ""}`}
          >
            <div className="hub-icon" style={{ background: "linear-gradient(135deg,#6c5ce7 0%,#00d2ff 100%)" }}>
              <Tv2 size={24} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-white font-extrabold text-lg leading-tight">NorthTV</div>
              <div className="text-white/55 text-xs mt-0.5 truncate">Chaînes en direct</div>
            </div>
            <span className="hub-chev">›</span>
          </button>
      ...
      {activeTab === "tv" && (
        <section className="w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 pt-4 pb-2" data-testid="tv-controls">
      ...
      {/* ===== Sports sub-tabs ===== */}
      {activeTab === "sports" && (
        <section className="w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 pt-4 pb-2" data-testid="sports-subtabs">
      ...
      {activeTab === "sports" && (
        <section className="w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 pt-3 pb-1">
          <SportsTicker />
        </section>
      )}

      {/* ===== Main content area (changes by tab) ===== */}
      <main className="w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 pb-20 pt-4">
        {activeTab === "tv" && (
          <>
            {!search.trim() && !onlyFavorites && recent.length > 0 && (
              <div className="mb-6" data-testid="recent-row">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="flex items-center gap-2 text-white font-extrabold text-lg">
                    <History size={18} className="text-[#ff2e63]" /> Continuer à regarder
                  </h2>
                  <button
                    type="button"
                    onClick={clearRecent}
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                  {recent.map((item) => (
                    <div key={item.id} className="group relative shrink-0 w-[150px]">
                      <button
                        type="button"
                        onClick={() => handlePickTv(item)}
                        className="w-full text-left glass rounded-xl overflow-hidden border border-white/10 hover:border-[#ff2e63]/50 transition-colors"
                      >
                        <div className="aspect-video bg-black/40 flex items-center justify-center">
                          {item.logo ? (
                            <img src={item.logo} alt="" className="max-h-full max-w-full object-contain" />
                          ) : (
                            <Tv2 className="text-white/20" size={28} />
                          )}
                        </div>
                        <div className="p-2">
                          ...
                          <div className="text-white/40 text-[10px] flex items-center gap-1 mt-0.5">
                            <Clock size={10} /> {item.country || item.source}
                          </div>
                        </div>
                      </button>
                      <button
                        ...
                        className="absolute top-1 right-1 w-6 h-6 rounded-full bg-black/70 text-white/70 hover:text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                        aria-label="Retirer"
                      >
                        <X size={12} />
                      </button>
        ...
        {activeTab === "north" && (
          <NorthTvTab onPick={handlePickNorth} />
        )}

        {activeTab === "frame" && (
          <FrameTvTab onPick={handlePickFrame} />
        )}
      </main>

      <footer className="w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 pb-10 text-center">
      ...
      {/* ===== North TV player (HLS or iframe fallback) ===== */}
      {northActive && northActive.stream_url && (
        <VideoPlayer
          channel={{ id: northActive.slug, name: northActive.name, country_code: "north" }}
          streamUrl={northActive.stream_url}
          onClose={() => setNorthActive(null)}
        />
      )}
      {northActive && !northActive.stream_url && northActive.iframe_url && (
        <IframePlayer
          src={northActive.iframe_url}
          title={`${northActive.name} — NorthTV`}
          onClose={() => setNorthActive(null)}
        />
      )}

      {/* ===== Frame TV player (HLS) ===== */}
      {frameActive && frameActive.stream_url && (
        <VideoPlayer
          channel={{ id: frameActive.id, name: frameActive.name, country_code: "frame" }}
          streamUrl={frameActive.stream_url}
          onClose={() => setFrameActive(null)}
        />
      )}
    </div>
  );
